...
interface RegistryEntry {
  name: string
  importPath: string
  props: RegistryProp[] | null
  parts?: Record<string, {name: string; props: RegistryProp[]; deprecated?: string}>
}

interface AiMetaPayload {
...
function AiMetaPanel() {
  const [payload, setPayload] = useState<AiMetaPayload | null>(null)

  useEffect(() => {
    const channel = addons.getChannel()
    const handler = (data: AiMetaPayload | null) => setPayload(data)
    channel.on(CHANNEL_EVENT, handler)
    return () => channel.off(CHANNEL_EVENT, handler)
  }, [])

  const meta = payload?.meta ?? null
  const registryEntry = payload?.registryEntry ?? null
  ...
  return (
    ...
    >
      {payload?.registryStale && (
        ...
        >
          registry stale — this meta changed since the last <code>pnpm duro:registry</code>; agents
          still see the old snapshot
        </div>
      )}
      {registryEntry && (
        <div style={{marginBottom: 16, fontFamily: 'monospace', fontSize: 12, color: '#888'}}>
          import {'{'}
          {registryEntry.name}
          {'}'} from '{registryEntry.importPath}'<span style={{margin: '0 8px'}}>·</span>
          npx @duro-app/cli {registryEntry.name}
        </div>
      )}
      ...
      {registryEntry && (registryEntry.props?.length || registryEntry.parts) && (
        <Section title="Props (extracted — what agents see)">
          {registryEntry.props && registryEntry.props.length > 0 && (
            <PropTable props={registryEntry.props} />
          )}
          {registryEntry.parts &&
            Object.values(registryEntry.parts).map((part) => (
              <div key={part.name} style={{marginBottom: 10}}>
                <Label>
                  {registryEntry.name}.{part.name}
                  {part.deprecated ? '  (deprecated)' : ''}
                </Label>
                {part.props.length > 0 ? (
                  <PropTable props={part.props} />
                ) : (
                  <div style={{color: '#666', fontSize: 12}}>no props</div>
                )}
              </div>
            ))}
        </Section>
      ...
      {meta.example && (
        <Section title="Canonical example">
          ...
          >
            <code>
              <HighlightedCode code={meta.example} />
            </code>
          </pre>
        </Section>
      )}
...
function PropTable({props}: {props: RegistryProp[]}) {
  return (
    <table style={{borderCollapse: 'collapse', fontSize: 12, fontFamily: 'monospace'}}>
      <tbody>
        {props.map((prop) => (
          <tr key={prop.name} style={{opacity: prop.deprecated ? 0.5 : 1}}>
            <td style={{padding: '2px 12px 2px 0', color: '#60a5fa', whiteSpace: 'nowrap'}}>
              {prop.name}
              {prop.required ? '*' : ''}
            </td>
            <td style={{padding: '2px 12px 2px 0', color: '#aaa'}}>{prop.type}</td>
            <td style={{padding: '2px 0', color: '#666', whiteSpace: 'nowrap'}}>
